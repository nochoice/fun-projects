input = `position=< 21992, -10766> velocity=<-2,  1>
...
parse = (input) => {
    const parser = /position=<(.*),(.*)> velocity=<(.*),(.*)>/;
    return input.split('\n')
                .map(line => line.match(parser))
                .map(spl => ({x: +spl[1],y: +spl[2],xV: +spl[3],yV: +spl[4]}));
};
...
normalizeBox = (data) => {
    ...
    return data.map(item => {
        item.x = (xMin < 0) ? item.x + Math.abs(xMin) : item.x;
        item.y = (yMin < 0) ? item.y + Math.abs(yMin) : item.y;
...
boundSize = (data) => {
    ...
    const xSize = Math.abs(Math.max(...xS) - Math.min(...xS));
    const ySize = Math.abs(Math.max(...yS) - Math.min(...yS));
...
isBoundSizeSmaller = (bs1, bs2) =>  (bs1.y <= bs2.y);

draw = (data, ratio = 1) => {
    const canvas = [];

    return data.reduce((acc, item) => {
        const y = Math.round(item.y / ratio);
...
fillDraw = (data) => {
    const xMax = Math.max(...data.map(item => item.length).filter(i => i));
...
move = (data, steps=1) => {
    return data.map(item => {
        return {
            x: item.x + (item.xV * steps),
            y: item.y + (item.yV * steps),
            xV: item.xV,
            yV: item.yV
        };
    });
...
show = (data) => {
    let dataActual = data;
    let dataNext = move(data);

    let bsActual = boundSize(dataActual);
    let bsNext = boundSize(dataNext);

    let count = 0;

    while (isBoundSizeSmaller(bsNext, bsActual)) {
        count++;
        bsActual = boundSize(dataActual);
        dataNext = move(dataActual);
        bsNext = boundSize(dataNext);
