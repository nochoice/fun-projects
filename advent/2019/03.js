const grid = {};
const sideEffectCounterForGrid = {};

const addDirection = (grid, pathName, position, path) => {
    const paths = path.split(',');

    console.log(pathName);

    paths.reduce((acc, p, i) => {
        addSingleDirection(grid, pathName, position, p)
        position = getNextPostion(position, p);
    }, {});

    return grid;
}

const addSingleDirection = (grid, pathName, position, path) => {
    let {direction, len} = parsePath(path);
    ...
    for (let i=0; i<len; i++) {
        x +=  direction[0];
        y +=  direction[1];
        pointToGrid(grid, `${x}|${y}`, pathName);
    }

    return grid;
}

const pointToGrid = (grid, position, pathName) => {
    if (!grid[position]) { 
        grid[position] = {};
        grid[position].paths = []
    };

    if (!sideEffectCounterForGrid[pathName]) {
        sideEffectCounterForGrid[pathName] = 0;
    }

    const wayLength = sideEffectCounterForGrid[pathName]++;

    grid[position].paths.push({pathName, wayLength});
...
const getNextPostion = (start, path) => {
    let {direction, len} = parsePath(path);
...
const crosses = (grid) => Object.keys(grid).filter((key) => new Set(grid[key].paths).size > 1);

const nearestManhattan = (positions) => {
...
const manhattanDistance = (grid, position, paths) => {
    paths.forEach((path, i) => addDirection(grid, i, position, path))
    return nearestManhattan(crosses(grid));
}


const result1 = manhattanDistance({}, [0,0], [
            ...
            console.log('03-1', result1);

exports.addSingleDirection = addSingleDirection;
exports.addDirection = addDirection;
exports.parsePath = parsePath;
exports.getNextPostion = getNextPostion;
